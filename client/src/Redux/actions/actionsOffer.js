...
export const addOffer = (offerBody,navigate) => async (dispatch) => {
    ...
    try {
      ...
      const resOffer= await axios.post('http://localhost:5000/api/offer', offerBody,{ headers: { Authorization: `Bearer ${token}`} })
      dispatch({
...
export const editOffer = (id, offerBody, navigate) => async (dispatch) => {
      const token=localStorage.getItem('token');
      try {
        const resOffer = await axios.put(`http://localhost:5000/api/offer/${id}`, offerBody,{ headers: { Authorization: `Bearer ${token}` }})
...
export const getOneOffer= (id) => async (dispatch) => {
        ...
        try {
      const res = await axios.get(`http://localhost:5000/api/offer/${id}`,{ headers: { Authorization: `Bearer ${token}` }})
 ...
 export const deleteOffer = (id) => async (dispatch) => {
          const token=localStorage.getItem('token');
          try {
            await axios.delete(`http://localhost:5000/api/offer/${id}`,{ headers: { Authorization: `Bearer ${token}` }})
            dispatch({
